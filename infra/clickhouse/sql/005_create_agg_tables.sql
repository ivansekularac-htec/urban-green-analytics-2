...
CREATE TABLE IF NOT EXISTS agg_daily_energy
(
    date_key UInt32,
    report_date Date,
...
CREATE TABLE IF NOT EXISTS agg_sensor_freshness
(
    farm_key UInt32,
    sensor_key UInt32,
    ...
    minutes_since_last_reading UInt32,

    loaded_at DateTime
)
ENGINE = MergeTree()
ORDER BY (farm_key, sensor_key);

CREATE TABLE IF NOT EXISTS agg_farm_sensor_health
(
    farm_key UInt32,

    total_sensor_count UInt32,
    active_sensor_count UInt32,
    inactive_sensor_count UInt32,

    coverage_health_index Float32,
