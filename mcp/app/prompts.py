...
1. Call read_resource with uri="{METRICS_URI}" and take the definition of
   "{metric}" as written there: its formula, its source tables, its unit, and
   whether higher or lower is better. Restate none of them from memory. If it
   is not defined there, say so and stop.
2. Call read_resource with uri="{CONVENTIONS_URI}" and apply it, in particular
   whether those tables need FINAL or argMax deduplication.
...
1. Call read_resource with uri="{METRICS_URI}" and match "{dimension}" to one
   of the metrics defined there. Use the closest match and name it in your
   answer. Take its formula, its unit, and whether higher or lower is better
   from the same entry.
...
2. Call read_resource with uri="{METRICS_URI}" and use the definition named
   "Sensor anomaly rate" for the trend. Do not write the ratio out from memory.
