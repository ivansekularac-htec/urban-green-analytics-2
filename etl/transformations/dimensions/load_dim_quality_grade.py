...
JOB_NAME = "load_dim_quality_grade"
...
def main() -> None:
    ...
    try:
        quality_grades_path = f"{settings.postgres_raw_root}/quality_grades/"

        # Read all source batches and keep the latest row
        # for each quality grade.
        quality_grades = latest_rows(
            read_full_table(
                spark,
                quality_grades_path,
            )
